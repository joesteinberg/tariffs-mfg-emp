...
#df_all_noadj_trans = load_results(t,6,2,0,1,0)
...
def plot_dyn(data,fname,leg=1,ylim=None,lr_bars=False,ylabel=None,size=(3.25,3.85)):
    ...
    if(leg):
        ax.legend(frameon=True, loc='best',fontsize=10, framealpha=1, edgecolor='white',borderpad=0,borderaxespad=1)
...
for s in range(5):
    ...
    plot_dyn(data,fname,ylim=(-2,5.5),lr_bars=True,ylabel='change in sectoral emp (percent total goods emp)',size=(3.25*0.6/0.4,3.85))        
    plt.close('all')
...
for s in range(5):
    ...
    plot_dyn(data,fname,lr_bars=True,ylabel='change in output (percent)',size=(3.25*0.6/0.4,3.85))
    plt.close('all')
...
for s in range(5):
    ...
    plot_dyn(data,fname,lr_bars=True,ylabel='change in investment (percent)',size=(3.25*0.6/0.4,3.85))
    plt.close('all')
...
for s in range(5):
    ...
    plot_dyn(data,fname,lr_bars=True,ylabel='change in relative capital cost (percent)',size=(3.25*0.6/0.4,3.85),ylim=(-5,10))
    plt.close('all')
...
for s in range(5):
    ...
    plot_dyn(data,fname,lr_bars=True,ylabel='change in relative material cost (percent)',size=(3.25*0.6/0.4,3.85),ylim=(-5,10))
    plt.close('all')
...
fname = 'fig_dyn_macro'
data = pd.DataFrame(data)
plot_dyn(data,fname,ylabel = 'percent change',size=(3.25*0.5/0.4,3.85),lr_bars=True)

# ATB tariffs w/o retaliation
data = {}
for s2 in range(4):
...
plot_dyn(data,fname,ylim=(-2,3),lr_bars=True,ylabel='change in sectoral emp (percent total goods emp)',size=(3.25*0.6/0.5,3.85))
...
plot_dyn(data,fname,ylim=(-2,3),lr_bars=True,ylabel='change in sectoral emp (percent total goods emp)',size=(3.25*0.6/0.5,3.85),leg=0)
...
# ATB tariffs w/ reversal + no frictions
#data = {}
#for s2 in range(4):
#    data[slabels[s2]] = diff_pct_lg(df_all_noadj_trans,'l%d'%s2)
#data['Total'] = pct_chg(df_all_noadj_trans,'lg')
#data = pd.DataFrame(data)
#fname = 'fig_dyn_labor_goods_atb_temp_noadj'
#plot_dyn(data,fname,ylim=(-2,3),lr_bars=True,ylabel='change in sectoral emp (percent total goods emp)',size=(3.25*0.6/0.5,3.85),leg=0)
...
ax.set_ylabel('Pct chg in goods emp')
fig.tight_layout()    
...
ax.set_ylim(-2,3.5)
ax.set_xlabel('Targeted sector')
ax.set_ylabel('change in total goods emp (percent)')
fig.tight_layout()    
